...
import { start, StartOption } from '../utils/start.js'
import { watch } from '../utils/watch/watch.js'

export default class Start extends Command {
  static description = 'Start fastify instance'

  static args = {
    entry: Args.string({ required: true, description: 'Entry point of fastify instance.' }),
  }

  static flags = {
    require: Flags.string({ char: 'r', description: 'Preload Modules, for example "-r ts-node/register".', multiple: true }),
    import: Flags.string({ char: 'i', description: 'Preload Modules, for example "-i ./script.mjs".', multiple: true }),
