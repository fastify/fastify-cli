...
const configFilePath = process.env.FASTIFY_CLI_CONFIG || path.join(cwd, 'fastify-cli.config.js')

if (fs.existsSync(configFilePath)) {
  const configFile = require(configFilePath)
  config = {
    ignore: configFile['ignore'] || config['ignore']
  }
}
...
const watch = function (args) {
  ...
  const watcher = chokidar.watch(cwd, { ignored: config['ignore'] })
